...
export class AddressController {
  ...
  @get('adresses/housenumber/{houseNumber}')
  @response(200, {
    description: 'House number find',
    content: {
      'application/json': {
        schema: getModelSchemaRef(Address, {includeRelations: true}),
      },
  ...
  async findByHouseNumber(
    @param.path.string('houseNumber') houseNumber: string,
  ): Promise<Address[] | null> {
    return this.addressRepository.findByHouseNumber(houseNumber);
  }

  @get('/addresses/streetname/{streetName}')
  @response(200, {
    description: 'Address model street name',
    content: {
      'application/json': {
        schema: getModelSchemaRef(Address, {includeRelations: true}),
      },
  ...
  async findByStreetName(
    @param.path.string('streetName') streetName: string,
  ): Promise<Address[] | null> {
    return this.addressRepository.findByStreetName(streetName);
  }
